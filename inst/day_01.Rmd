...
fix_written <- \(dt) {
  ...
  dt[, n := #I'm sure there's better ways to do this. My brain feels like cottage cheese.
       stringr::str_replace_all(n, 'one', '1')]
  dt[, n :=
       stringr::str_replace_all(n, 'two', '2')]
  dt[, n :=
       stringr::str_replace_all(n, 'three', '3')]
  dt[, n :=
       stringr::str_replace_all(n, 'four', '4')]
  dt[, n :=
       stringr::str_replace_all(n, 'five', '5')]
  dt[, n :=
       stringr::str_replace_all(n, 'six', '6')]
  dt[, n :=
       stringr::str_replace_all(n, 'seven', '7')]
  dt[, n := #seriously, it's R, there's GOT to be utilities to make this conversion already
       stringr::str_replace_all(n, 'eight', '8')]
  dt[, n := 
       stringr::str_replace_all(n, 'nine', '9')]
  dt
}
```
